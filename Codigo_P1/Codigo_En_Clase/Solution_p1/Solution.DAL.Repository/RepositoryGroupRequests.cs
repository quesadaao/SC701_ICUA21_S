...
namespace Solution.DAL.Repository
{
    ...
    public class RepositoryGroupRequests : Repository<GroupRequests>, IRepositoryGroupRequests
    {
        //Constructor correspondiente a la clase
        //Parametro Context para poderlo recibir y utilizar
        public RepositoryGroupRequests(SolutionDBContext solutionDBContext) : base(solutionDBContext)
        {
        ...
        public async Task<IEnumerable<GroupRequests>> GetAllWithAsync()
        {
            return await context.GroupRequests.Include(m => m.Group).ToListAsync();
        }

        // Metodo encargado de traer datos con relaciones
        // Palabra clave include donde hacer la referencia entre ambas tablas
        // Solo trae un objeto
        public async Task<GroupRequests> GetOneByIdWithAsync(int id)
        {
            return await context.GroupRequests.Include(m => m.Group).SingleOrDefaultAsync(m => m.GroupRequestId == id);
        }
